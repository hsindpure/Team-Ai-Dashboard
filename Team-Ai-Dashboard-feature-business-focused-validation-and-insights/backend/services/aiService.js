...
class AIService {
    ...
    async getSuggestions(schema, sampleData, userContext = null) {
      ...
      console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log(`🎯 [${requestId}] AI SUGGESTION REQUEST`);
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('Has user context:', !!userContext);
      console.log('Measures:', schema.measures?.length || 0);
      console.log('Dimensions:', schema.dimensions?.length || 0);
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
      ...
      try {
        ...
        // ✅ Skip cache if user has context (personalized request)
        if (!userContext && this.cache.has(cacheKey)) {
          console.log(`✅ [${requestId}] Using cached suggestions`);
        ...
        console.log(`🤖 [${requestId}] Calling AI service...`);
        
        // Use sample data for AI analysis instead of full dataset
        const optimizedSchema = this.optimizeSchemaForAI(schema, sampleData);
        
        // ✅ CRITICAL: Single AI call with proper error handling
        const suggestions = await this.getAISuggestions(optimizedSchema, sampleData, userContext);
        
        console.log(`✅ [${requestId}] AI SUCCESS`);
        console.log(`📊 Generated: ${suggestions.kpis?.length || 0} KPIs, ${suggestions.charts?.length || 0} charts`);
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
        ...
        // ✅ ONLY reach here if AI truly failed
        console.error(`❌ [${requestId}] AI FAILED:`, error.message);
        console.log(`🔄 [${requestId}] Using fallback suggestions...`);
        
        const fallbackSuggestions = this.getEnhancedFallbackSuggestions(schema, sampleData);
        
        console.log(`✅ [${requestId}] FALLBACK GENERATED`);
        console.log(`📊 Generated: ${fallbackSuggestions.kpis?.length || 0} KPIs, ${fallbackSuggestions.charts?.length || 0} charts`);
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    ...
    async getAISuggestions(optimizedSchema, sampleData) {
      ...
      console.log(`🤖 AI Response Recived Sucessfully`);
      if (!content) {
...
4. **PROVIDE BUSINESS INSIGHTS:**
   ...
   **CRITICAL - SUPPORTED CHART TYPES ONLY:**
You MUST ONLY use these 5 chart types. Do NOT suggest any other chart types:
1. "bar" - Bar Chart (for comparing categories)
2. "line" - Line Chart (for trends over time)
3. "area" - Area Chart (for cumulative trends)
4. "pie" - Pie Chart (for proportions, max 8 categories)
5. "scatter" - Scatter Plot (for correlation between two measures)


**ADVANCED CHARTS (Multiple Measures):**
6. "stacked-bar" - Stacked Bar Chart (for part-to-whole comparison, requires 2+ measures)
7. "grouped-bar" - Grouped Bar Chart (for side-by-side comparison, requires 2+ measures)
8. "heatmap" - Heatmap (for intensity visualization, 1 measure + 1-2 dimensions, max 200 cells)

**USAGE RULES:**
- Use "bar", "line", "area", "pie" when you have ONE measure
- Use "stacked-bar" or "grouped-bar" when you have MULTIPLE measures to compare
- Use "scatter" when you need to show correlation between TWO numeric measures
- Use "heatmap" when showing intensity/density patterns across dimensions
- "pie" ONLY for 2-8 categories (not for time series or high cardinality)
- "heatmap" ONLY when both dimensions have < 20 unique values each

❌ DO NOT USE: combo, waterfall, funnel, gauge, radar, box plot, histogram, geo map, treemap
✅ ONLY USE: bar, line, area, pie, scatter, stacked-bar, grouped-bar, heatmap
