...
const deduplicateRequest = async (key, requestFn) => {
  ...
  if (activeRequests.has(key)) {
    console.log('🔄 Request already in progress, reusing:', key.substring(0, 50) + '...');
    return activeRequests.get(key);
  }

  console.log('🚀 New request initiated:', key.substring(0, 50) + '...');
  ...
  const promise = requestFn()
    .finally(() => {
      ...
      console.log('✅ Request completed and removed from cache');
    });
...
export const generateDashboard = async (
  ...
  try {
    ...
    const cacheKey = `${endpoint}-${sessionId}-${JSON.stringify(filters)}-${dataLimit}-${!!userContext}`;
    ...
    return deduplicateRequest(cacheKey, async () => {
      ...
      console.log('✅ Dashboard response received:', {
        ...
        kpis: response.data.dashboard?.kpis?.length || response.data.data?.kpis?.length || 0,
        charts: response.data.dashboard?.charts?.length || response.data.data?.charts?.length || 0
      });

      // ✅ Show warning if fallback was used
      if (response.data.usedFallback) {
        console.warn('⚠️ Fallback suggestions were used');
      }
