...
const Dashboard = ({ 
  ...
  const loadInitialDashboard = useCallback(async () => {
    ...
    if (loadingRef.current) {
      console.log('⏭️ Initial dashboard load already in progress, skipping...');
      return;
    }
    ...
    try {
      ...
      console.log('Is large dataset:', !!fileInfo?.isLargeDataset);
      console.log('═══════════════════════════════════════════════════\n');
      ...
      console.log('Used fallback:', result.usedFallback || false);
      console.log('KPIs:', result.dashboard?.kpis?.length || 0);
      console.log('Charts:', result.dashboard?.charts?.length || 0);
      console.log('═══════════════════════════════════════════════════\n');
      
      // ✅ Show appropriate message based on source
      if (result.usedFallback) {
        message.warning({
          content: 'AI service unavailable. Using basic dashboard suggestions.',
          duration: 5,
          key: 'fallback-warning'
        });
      } else if (initialDataLimit) {
        message.info({
          content: `Dashboard loaded with ${initialDataLimit.toLocaleString()} record limit for optimal performance`,
          duration: 3,
          key: 'limit-info'
        });
      } else {
      ...
      console.error('\n❌ ═══════════════════════════════════════════════════');
      console.error('💥 INITIAL DASHBOARD LOAD FAILED');
      console.error('═══════════════════════════════════════════════════');
  ...
  }, [sessionId, fileInfo, userContext]);
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      console.log('🔄 Triggering dashboard load (debounced)...');
  ...
  }, [sessionId, loadInitialDashboard, loadSessionData]);
...
const result = await generateDashboard(
  ...
  userContext  // ✅ ADD THIS PARAMETER
      ...
      loadingRef.current = false;  // ✅ ADD THIS LINE
    }
